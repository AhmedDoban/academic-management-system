...
const Studetn_Login = async (Req, Res) => {
  ...
  try {
    ...
    const USER = await Student_Model.findOne({ national_ID });
    if (USER === null) {
...
const Studetn_Register = async (Req, Res) => {
  ...
  try {
    // Searching in the database with email may be email is wrong
    const Check_Student = await Users_Model.findOne({ national_ID });
    if (Check_Student) {
      return Res.json({
        Status: Codes.FAILD,
        Status_Code: Codes.FAILD_CODE,
      ...
      Student.Token = await JWT.Genetate_Token(USER);
      await Student.save();
